...
/** All fail findings for the currently-mounted chart, across every check. */
async function gateFails(chartId = ''): Promise<string[]> {
  await session.page.addScriptTag({ path: measureBundle });
  return session.page.evaluate((cid) => {
    const svg = document.querySelector('svg.gc-chart') as SVGSVGElement;
    return (window as unknown as GateGlobal).geekchartMeasure.measureChart(svg, {
      chartId: cid,
    }).fails;
  }, chartId);
}

/** Fail findings from one named check, for a test pinning a specific rule
 *  rather than sweeping the whole gate. */
async function gateCheck(id: string, chartId = ''): Promise<string[]> {
  await session.page.addScriptTag({ path: measureBundle });
  return session.page.evaluate(
    ([checkId, cid]) => {
      const svg = document.querySelector('svg.gc-chart') as SVGSVGElement;
      return (window as unknown as GateGlobal).geekchartMeasure
        .runCheck(svg, checkId, { chartId: cid })
...
describe('routing', () => {
  ...
  test('DESIGN.md gate parity: every graph-family fixture, every check, no fail findings', async () => {
    // This used to be roughly a dozen separate hand-rolled reimplementations
    // of the gate's own arithmetic: bends, detours, crossings, channel
    // centring, arrival side, self-pierce, hairpins, sole-child centring,
    // parallel clearance, shared rows, panel hugging and orphan columns among
    // them (each duplicated the matching check in packages/cli/src/measure/,
    // slightly differently). They now all call the same library the gate
    // calls, so a rule is defined once, and this proves the drawn pipeline
    // still meets every one of them, fixture by fixture.
    //
    // Rendered with motion off: every check assumes the *finished* frame
    // (gate.mjs freezes the gallery's animation the same way before it
    // measures), and an animated mount can catch a panel box mid-reveal —
    // still at opacity 0, so `visible()` drops it and its contents read as
    // stray boxes rather than a panel's own.
    if (!existsSync(measureBundle)) {
      throw new Error('run `pnpm --filter @geekchart/cli build` first');
    }
    // One pre-existing gap, out of scope for a library-extraction refactor —
    // fixing the renderer belongs to a separate change that names the
    // DESIGN.md rule and the measured before/after, not to this one: under
    // this suite's manim scene (not the gate's own default-scene render),
    // flow.mmd's B (a decision) and E (two hops downstream) share a vertical
    // band without sharing a row. B's diamond is tall enough to hold its own
    // two children's centres but, eight units short, not quite E's — E sits
    // in a row of its own sibling C's instead.
    const KNOWN_GAPS: Record<string, string[]> = {
      'flow.mmd': ['2.3 1 node pairs overlapping bands but not sharing a row (B~E)'],
    };
    for (const name of GRAPH_FIXTURES) {
      await mount(readFileSync(join(fixtures, name), 'utf8'), { motion: false });
      const chartId = name.replace(/^blog\//, '').replace(/\.mmd$/, '');
      const fails = await gateFails(chartId);
      const unexpected = fails.filter((f) => !(KNOWN_GAPS[name] ?? []).includes(f));
      assert.deepEqual(unexpected, [], `${name}: ${unexpected.join('; ')}`);
    }
  });
});
...
describe('pie, mindmap and git graph', () => {
  ...
  test('a git graph lane spans only its own content, never the full canvas', async () => {
    ...
    // ran well past their last commit. Calls the gate's own 10.5-lane-overrun
    // check (packages/cli/src/measure/charts.ts) instead of reimplementing it.
    await mount(readFileSync(join(fixtures, 'gitgraph.mmd'), 'utf8'), { motion: false });
    const fails = await gateCheck('10.5-lane-overrun');
  ...
  test('a git graph connector never rides along a lane for more than 16 units', async () => {
    ...
    // crossed. Calls the gate's own 6.4-lane-ride check
    // (packages/cli/src/measure/charts.ts) instead of reimplementing it.
    await mount(readFileSync(join(fixtures, 'gitgraph.mmd'), 'utf8'), { motion: false });
    const fails = await gateCheck('6.4-lane-ride');
    assert.deepEqual(fails, [], fails.join('; '));
  });
