...
function attemptDraw(
  ...
  const svg =
    `<svg class="gc-chart" data-gc="${uid}" data-flow="${graph.direction}"${graph.engine === 'channels' ? ' data-gc-engine="channels"' : ''}${graph.layoutKind ? ` data-gc-layout="${graph.layoutKind}"` : ''} data-label-count="${labelCount}" viewBox="${viewBox}" role="img" xmlns="${SVG}">` +
